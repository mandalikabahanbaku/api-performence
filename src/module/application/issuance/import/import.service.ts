...
import {
    IssuanceImportPreviewDTO,
    IssuanceImportRowSchema,
    ResponseIssuanceImportDTO,
...
export class IssuanceImportService {
    static async preview(rows: Record<string, unknown>[]): Promise<ResponseIssuanceImportDTO> {
        ...
        const validCodes = parsed.filter((r) => r.success).map((r) => r.data["PRODUCT CODE"]);
    ...
    private static async bulkInsert(
        ...
        for (const d of data) {
            ...
            if (Number.isInteger(quantity)) {
                summary.set(d.code, (summary.get(d.code) || 0) + (quantity as number));
            }
        }

        const codes = Array.from(summary.keys());
        const quantities = Array.from(summary.values());

        if (codes.length !== data.length) {
            logger.warn("Some issuance rows were aggregated or dropped", {
                total_original: data.length,
                total_unique: codes.length,
            });
        }
