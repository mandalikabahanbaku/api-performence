...
export class BOMService {
    static async list(query: QueryBOMDTO): Promise<ResponseBOMListDTO> {
        ...
        rows.forEach((r) => {
            ...
            const sizeFactor = r.use_size_calc ? 100 : 1;
            ...
            group.items.push({
                ...
                needs_to_buy: itemForecast.map((f: any, idx: number) => {
                    ...
                    return {
                        ...
                        month: f.month,
                        year: f.year,
                        value: Math.floor(Math.round(qty) * r.recipe_qty * sizeFactor),
                    };
                }),
                safety_stock_x_bom: Math.floor(Math.round(group.safety_stock) * r.recipe_qty * sizeFactor),
                need_produce_x_bom: group.need_produce.map((np: any) => ({
                    period: np.period,
                    month: np.month,
                    year: np.year,
                    value: np.value * r.recipe_qty * sizeFactor,
                })),
            });
        });
        ...
        if (rawMat) {
            ...
            const details = recipes.map((r) => {
                ...
                forecastRange.forEach((p, idx) => {
                    ...
                    const req = Math.floor(effectiveQty * Number(r.quantity) * sizeFactor);
                ...
                const productNeedProduce = forecastRange.map((p) => {
                    ...
                    const val = needAtMonth * Number(r.quantity) * sizeFactor;
                ...
                const productSalesHistory = salesRange.map((s) => {
                    ...
                    const val = Math.floor(sVal * Number(r.quantity) * sizeFactor);
                    return {
                        period: `${s.month}/${s.year}`,
                        month: s.month,
                        year: s.year,
                ...
                return {
                    ...
                    safety_stock: Math.floor(productSS * Number(r.quantity) * sizeFactor),
